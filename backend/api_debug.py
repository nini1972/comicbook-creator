...
print("Starting API server with debugging...")

try:
    print("Importing CrewAI modules...")
...
async def run_crew_stream(topic: str, stream_callback):
    ...
    try:
        ...
        print(f"Creating VisualComicCrew for topic: {topic}")
        ...
        print(f"Error in run_crew_stream: {e}")
        import traceback
        traceback.print_exc()
...
async def generate_comic(request: Request, topic: str = "A cat who wants to fly"):
    ...
    """
    print(f"Received request to generate comic with topic: {topic}")
    ...
    async def event_generator():
        ...
        task = asyncio.create_task(run_crew_stream(topic, stream_callback))
